...
| `device_users` admin recebia `sessionType: operational` no login online | `loginDeviceUser` hardcodava `sessionType: 'operational'` para todos da tabela `device_users`, ignorando o campo `isDeviceAdmin`; `applyAdminBypass()` nunca era chamado no fluxo online | `device-user-auth.service.ts` linha 54: `sessionType: operationalUser.isDeviceAdmin ? 'admin' : 'operational'` |
| Sessão remota orfã ao navegar para outra página | Frontend não fechava sessão ao sair da página do device | `useEffect` cleanup com `remoteSessionRef` fecha sessão ao desmontar; botão muda estado (roxo ↔ vermelho); confirmação antes de encerrar |
| Fechar sessão remota falhava se device já tinha desconectado | `close()` lançava `BadRequestException` se sessão já CLOSED pelo gateway | `close()` agora é idempotente — retorna sessão se já fechada |
| Sessões PENDING/ACTIVE orfãs nunca eram limpas | Sem cleanup automático no backend | Timer a cada 15s: PENDING > 30s e ACTIVE > 5min sem device são auto-fechadas |
| Acesso remoto loading infinito com tela desligada | `RemoteViewService` usava `PARTIAL_WAKE_LOCK` (só CPU) — tela não ligava | `FULL_WAKE_LOCK + ACQUIRE_CAUSES_WAKEUP` no serviço liga a tela e mantém ligada durante a sessão |
| H.264 voltava ao loading após aprovar permissão com celular bloqueado | Keyguard (tela de bloqueio) voltava ao foreground após `RemoteAccessActivity.finish()`, interrompendo pipeline | `setKeyguardDisabled(true)` via Device Owner ao iniciar sessão; `setKeyguardDisabled(false)` ao encerrar; `RemoteAccessActivity` com `setShowWhenLocked(true)` + `setTurnScreenOn(true)` |
