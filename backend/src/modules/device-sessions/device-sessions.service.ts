...
export class DeviceSessionsService {
  ...
  async findById(sessionId: string, deviceId: string): Promise<DeviceSession | null> {
    return this.sessionRepository.findOne({ where: { id: sessionId, deviceId } })
  }

  async closeActiveForUser(tenantId: string, deviceUserId: string, reason: string): Promise<void> {
    await this.sessionRepository
      .createQueryBuilder()
      .update(DeviceSession)
      .set({ status: DeviceSessionStatus.INTERRUPTED, endedAt: new Date(), endedReason: reason })
      .where('tenantId = :tenantId', { tenantId })
      .andWhere('deviceUserId = :deviceUserId', { deviceUserId })
      .andWhere('status = :status', { status: DeviceSessionStatus.ACTIVE })
      .execute()
  }
